...
cap = cv2.VideoCapture("D:\PycharmProjects\MiniProject\\testvid.mp4")
...
# We don't consider an initial detection unless it's detected 7 times, this gets rid of false positives
detection_thresh = 7
...
output_path = "D:/PycharmProjects/MiniProject/Recordings/"
...
while True:
    ...
    # If we have consecutively detected a person 7 times then we are sure that someone is present
    if sum(de) >= detection_thresh and not status:
        status = True
        entry_time = datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S")
